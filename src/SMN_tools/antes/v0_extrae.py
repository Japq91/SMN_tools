...
def extrac_ETA(gribfile):
    ...
    # Isobáricas
    uprs = xr.open_dataset(gribfile, engine='cfgrib',
                           backend_kwargs={'filter_by_keys': {'typeOfLevel': 'isobaricInhPa', 'shortName': 'u'}})
    vprs = xr.open_dataset(gribfile, engine='cfgrib',
                           backend_kwargs={'filter_by_keys': {'typeOfLevel': 'isobaricInhPa', 'shortName': 'v'}})
    tprs = xr.open_dataset(gribfile, engine='cfgrib',
                           backend_kwargs={'filter_by_keys': {'typeOfLevel': 'isobaricInhPa', 'shortName': 't'}})
    # Superficie
    landsfc = xr.open_dataset(gribfile, engine='cfgrib',
                              backend_kwargs={'filter_by_keys': {'typeOfLevel': 'surface', 'shortName': 'lsm'}})
    pressfc = xr.open_dataset(gribfile, engine='cfgrib',
                              backend_kwargs={'filter_by_keys': {'typeOfLevel': 'surface', 'shortName': 'sp'}})
    # Presión al nivel del mar
    prmsl = xr.open_dataset(gribfile, engine='cfgrib',
                            backend_kwargs={'filter_by_keys': {'typeOfLevel': 'meanSea', 'shortName': 'mslet'}})
    # Viento 10m
    v10m = xr.open_dataset(gribfile, engine='cfgrib', backend_kwargs={'filter_by_keys': {'paramId': 166}})
    u10m = xr.open_dataset(gribfile, engine='cfgrib', backend_kwargs={'filter_by_keys': {'paramId': 165}})
    # 2m
    d2m = xr.open_dataset(gribfile, engine='cfgrib',  backend_kwargs={'filter_by_keys': {'paramId': 168}})
    t2m = xr.open_dataset(gribfile, engine='cfgrib',  backend_kwargs={'filter_by_keys': {'paramId': 167}})
#'''
...
def extrac_ETA(gribfile):
    ...
    # Viento 10m
    v10m = xr.open_dataset(gribfile, engine='cfgrib',
                           backend_kwargs={
                               'filter_by_keys': {'paramId': 166},
                               'indexpath': ''
                           })
    u10m = xr.open_dataset(gribfile, engine='cfgrib',
                           backend_kwargs={
    ...
    # 2m
    d2m = xr.open_dataset(gribfile, engine='cfgrib',
                          backend_kwargs={
                              'filter_by_keys': {'paramId': 168},
                              'indexpath': ''
                          })
    t2m = xr.open_dataset(gribfile, engine='cfgrib',
                          backend_kwargs={
